...
def choose_channels(labels, event_id, args):
    ...
    channels = labels.loc[labels["ID"] == event_id, "Channel"].drop_duplicates().tolist()
    return channels[: args.max_channels_per_event]
...
def plot_event(df, labels, event, channels, output_path):
    n = len(channels)
    fig, axes = plt.subplots(n, 1, figsize=(12, max(3.0, 2.2 * n)), sharex=True)
    if n == 1:
        axes = [axes]

    event_labels = labels[labels["ID"] == event["ID"]]
    for ax, channel in zip(axes, channels):
        ax.plot(df["timestamp"], df[channel], linewidth=0.9, color="#2f5d8c")
        ...
        for row in rows.itertuples():
            ...
            if duration_seconds <= 1:
                ...
                ax.axvline(row.StartTime, color="#8c1d18", linewidth=0.65, alpha=0.45)
                ax.axvline(row.EndTime, color="#8c1d18", linewidth=0.65, alpha=0.45)
        label_col = f"is_anomaly_{channel}"
        ymin, ymax = ax.get_ylim()
        if point_times:
            marker_y = ymax - (ymax - ymin) * 0.04
            ax.scatter(point_times, [marker_y] * len(point_times), marker="v", s=42,
            ...
            marker_y = ymax - (ymax - ymin) * 0.04
            ax.scatter(anom["timestamp"], [marker_y] * len(anom), marker="|", s=18,
                       color="#8c1d18", alpha=0.45, zorder=5)
        ax.set_ylabel(channel)
        ax.grid(True, axis="y", linewidth=0.4, alpha=0.35)

    axes[0].set_title(event_title(event), fontsize=10)
